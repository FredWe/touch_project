...
if [ $stage -le 0 ]; then
  gmm-est --binary=false --min-gaussian-occupancy=3  --mix-up=$numgauss --power=$power \
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $stage -le $x ]; then
    ...
    $cmd $dir/log/update.$x.log \
      gmm-est --binary=false --write-occs=$dir/$[$x+1].occs --mix-up=$numgauss --power=$power $dir/$x.mdl \
...
( cd $dir; rm final.{mdl,occs} 2>/dev/null; 
ln -s $x.mdl final.mdl; ln -s $x.occs final.occs )
